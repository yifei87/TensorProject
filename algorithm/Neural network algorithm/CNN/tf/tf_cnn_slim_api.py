...
def convolutional_neural_network(data):
    ...
    '''
    net = tf.reshape(data, [-1, 28, 28, 1])
    net=slim.conv2d(net,32,5,stride=1,padding='same',activation_fn=tf.nn.relu)
    net=slim.max_pool2d(net,2,2,padding='same')
    net=slim.dropout(net,0.8)
    net = slim.conv2d(net, 64, 5, stride=1, padding='same', activation_fn=tf.nn.relu)
    net = slim.max_pool2d(net, 2, 2, padding='same')
    net = slim.dropout(net, 0.8)
    net=slim.flatten(net)
    net=slim.fully_connected(net,1024,activation_fn=tf.nn.relu)
    net = slim.dropout(net, 0.8)
    output=slim.fully_connected(net,n_output_layer,activation_fn=None)
