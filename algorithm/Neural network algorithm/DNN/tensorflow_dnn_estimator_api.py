...
# 估计器的输入函数
'''
我们必须提供返回数据的函数，而不是直接向Estimator提供原始数据。 
这使数据源具有更大的灵活性，以及数据如何随机混洗和迭代。
请注意，我们将使用DNNClassifier创建一个Estimator，它假定类数是整数，
所以我们使用data.train.cls(非one_hot编码)而不是data.train.labels(一个one_hot编码数组)。
该函数还具有用于batch_size，queue_capacity和num_threads的参数，以更好地控制数据读取。 
在我们的例子中，我们直接从内存中的numpy数组中获取数据，所以不需要。'''
...
'''
'''
同样，我们需要创建一个读取测试集数据的函数。 请注意，我们只想处理这些图像一次，
因此num_epochs = 1，我们不希望图像混乱，因此shuffle = False。
'''
