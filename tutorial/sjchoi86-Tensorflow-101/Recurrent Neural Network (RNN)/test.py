...
with tf.variable_scope('rnnlm'):
    ...
    with tf.device("/cpu:0"):
        embedding = tf.get_variable("embedding", [vocab_size, rnn_size]) # [98,128]
        inputs = tf.split(tf.nn.embedding_lookup(embedding, input_data), seq_length,1) # [50,200,128]-->200个[50,128]序列
        inputs = [tf.squeeze(_input, [1]) for _input in inputs] # [200,50,128] ,200个[50,128]序列
# Output
def loop(prev, _):
